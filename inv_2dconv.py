...
k = np.random.normal(0, 1, size=(3,3))

# compute convolutions. (?? No flip ??)
conv 	= signal.convolve(		A, k, mode="same")
fftconv = signal.fftconvolve(	A, k, mode="same")
fft		= ifftn( fftn( A, (2*10-1,2*10-1) ) * fftn( k, (2*10-1,2*10-1)), (2*10-1, 2*10-1) ).real[1: 11, 1: 11]
...
for j in range(3): 
	ax[0, j].imshow(lst[j].reshape(10, 10))
...
for i in range(3):
	for j in range(3): 
		...
		ax[i+1,j].imshow(np.abs(lst[i] - lst[j]).reshape(10, 10)) # be sure to each check size are not relative. 
...
A_part 	= fftn( A, (2*10-1, 2*10-1)) 
k_part 	= fftn( k, (2*10-1, 2*10-1))
inside 	= A_part * k_part
conv	= ifftn( inside, (2*10-1, 2*10-1) )
 
rec 	= fftn( conv, (2*10-1, 2*10-1 ) )
assert np.allclose(inside, rec)

rec		= rec * 1 / k_part
assert np.allclose(A_part, rec)

rec		= ifftn( rec, (2*10-1, 2*10-1 ))
assert np.allclose( rec[:10, :10].real, A )

fig, ax = plt.subplots(1, 4)

ax[0].imshow(A)
ax[0].set_title("A")
ax[1].imshow(conv.reshape(19, 19).real)
ax[1].set_title("conv (all)")
ax[2].imshow(rec.reshape(19, 19).real)
ax[2].set_title("rec (all)")
ax[3].imshow(rec[0: 10, 0:10].reshape(10, 10).real)
ax[3].set_title("rec")
